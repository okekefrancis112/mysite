...
def post_share(request, post_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # ... send email
            from_ = "okekefrancis112@gmail.com"
            password = "uhoqhaygeucywapl"
            subject_ = '{} ({}) recommends you reading " {} "'.format(cd['name'], cd['email'], post.title)
            message = 'Read "{}" at {}\n\n{}\'s comments:{}'.format(post.title, post_url, cd['name'], cd['comments'])
            yag = yagmail.SMTP(from_, password)
            yag.send(subject= subject_, contents= message , to= [cd['to']])
            sent = True
    else:
        form = EmailPostForm()
